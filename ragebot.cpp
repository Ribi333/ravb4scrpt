...
bool c_ragebot::can_fire(bool ignore_revolver)
{
	...
	if (HACKS->weapon_info->weapon_type >= WEAPONTYPE_PISTOL && WEAPONTYPE_MACHINEGUN <= 6 && HACKS->weapon->clip1() < 1)
		return false;
...
void c_ragebot::update_hitboxes()
{
    ...
    // FORCE BODY AIM CONDITIONS
    bool force_body = g_cfg.binds[force_body_b].toggled;
    
    // Check if we've missed this player multiple times on head
    LISTENER_ENTITY->for_each_player([&](c_cs_player* player) {
        if (!player->is_alive() || player->dormant()) return;
        
        auto idx = player->index();
        if (missed_shots[idx] >= 2) { // After 2 misses, prefer body
            force_body = true;
        }
    });
    ...
    // Normal selection - but prioritize body for reliability
    if (rage_config.hitboxes & stomach)
        hitboxes.emplace_back(HITBOX_STOMACH);
    if (rage_config.hitboxes & pelvis)
        hitboxes.emplace_back(HITBOX_PELVIS);
...
int c_ragebot::estimate_body_damage(c_cs_player* player)
{
    auto local_anims = ANIMFIX->get_local_anims();
    
    // Fix the get_hitbox_position call - check how many parameters it needs
    vec3_t stomach_pos;
    
    // Try one of these depending on your function signature:
    // Option 1: If it needs matrix parameter
    // stomach_pos = player->get_hitbox_position(HITBOX_STOMACH, some_matrix);
    
    // Option 2: If it's a different function name
    // stomach_pos = player->hitbox_position(HITBOX_STOMACH);
    
    // Option 3: Simple fallback
    stomach_pos = player->get_abs_origin();
    
    auto bullet = penetration::simulate(HACKS->local, player, local_anims->eye_pos, stomach_pos);
...
{
	if (!rage_config.auto_scope || HACKS->convars.weapon_accuracy_nospread->get_int() != 0)
		return;
...
static void player_move(c_cs_player* player, anim_record_t* record)
{
...
static bool start_fakelag_fix(c_cs_player* player, anims_t* anims)
{
	...
	if (predicted_tick > 0 && predicted_tick < 20 || EXPLOITS->cl_move.shifting)
	{
		...
		{
			// BE MORE CONSERVATIVE WITH EXTRAPOLATION
			if (prediction_ticks > 2) // Limit to 2 ticks max
				prediction_ticks = 2;
				
			// Don't extrapolate if player is moving fast
			if (record->prediction.velocity.length_2d() > 150.0f)
				return false;
...
static void pre_cache_centers(int damage, std::vector<int>& hitboxes, vec3_t& predicted_eye_pos, rage_player_t* rage)
{
	rage->reset_hitscan();
	auto anim = ANIMFIX->get_local_anims();
...
static void get_result(bool& out, const vec3_t& start, const vec3_t& end, rage_player_t* rage, int hitbox, matrix3x4_t* matrix, anim_record_t* record)
{
}

static auto fast_rsqrt = [](float a, float* out) {
...
static bool hitchance(vec3_t eye_pos, rage_player_t& rage, const rage_point_t& point, anim_record_t* record, const float& chance, matrix3x4_t* matrix, float* hitchance_out = nullptr)
{
	static auto weapon_accuracy_nospread = HACKS->convars.weapon_accuracy_nospread;
	if (weapon_accuracy_nospread && weapon_accuracy_nospread->get_bool())
		return true;
	...
	// GAMESENSE OPTIMIZATION: Calculate required hits once
	int needed_hits = (int)std::ceil((chance * MAX_SEEDS));
	...
	for (int i = 0; i < MAX_SEEDS; i++)
	{
		...
		// GAMESENSE OPTIMIZATION: Early success exit
		if (current >= needed_hits) {
			rage.restore.restore(rage.player);
			if (hitchance_out)
				*hitchance_out = 1.0f;
			return true;
		...
		// GAMESENSE OPTIMIZATION: Early failure exit
		if ((MAX_SEEDS - i + current) < needed_hits) {
			rage.restore.restore(rage.player);
			if (hitchance_out)
...
static void collect_damage_from_multipoints(int damage, vec3_t& predicted_eye_pos, rage_player_t* rage, rage_point_t& points, anim_record_t* record, matrix3x4_t* matrix_to_aim, bool predicted)
{
...
{
    if (rage->points_to_scan.empty() || !rage->start_scans || !rage->hitscan_record)
        return;

    rage->restore.store(rage->player);
    
    auto matrix_to_aim = rage->hitscan_record->extrapolated ? 
        rage->hitscan_record->predicted_matrix : rage->hitscan_record->matrix_orig.matrix;
    LAGCOMP->set_record(rage->player, rage->hitscan_record, matrix_to_aim);

    auto dmg = this->get_min_damage(rage->player);
    int lethal_threshold = rage->player->health();
    
    // Process center points first, add multipoints only if needed
    for (auto& points : rage->points_to_scan) {
        // Skip if we already have lethal damage
        if (points.damage >= lethal_threshold) {
            continue; // This point is already good enough
        }
        
        // SYNCHRONOUS multipoint processing instead of threading
        collect_damage_from_multipoints_sync(dmg, predicted_eye_pos, rage, points, 
                                           rage->hitscan_record, matrix_to_aim, points.predicted_eye_pos);
    }
...
void c_ragebot::scan_players()
{
    ...
    valid_players.reserve(32);
    
    // Quick first pass - collect only promising targets
    LISTENER_ENTITY->for_each_player([&](c_cs_player* player)
    {
        if (!player->is_alive() || player->dormant() || player->has_gun_game_immunity())
            return;
        ...
        // Quick distance check - skip very far players early
        if (rage->distance > 3000.f)
            return;
    ...
    // Process players in order of distance (closest first for better performance)
    std::sort(valid_players.begin(), valid_players.end(), [&](c_cs_player* a, c_cs_player* b) {
        return rage_players[a->index()].distance < rage_players[b->index()].distance;
    });

    // Process maximum 2 players per tick to reduce lag
    int processed = 0;
    for (auto* player : valid_players) {
        ...
        // REMOVE THREADING - process synchronously for better performance
        pre_cache_centers(dmg, hitboxes, predicted_eye_pos, rage);
        do_hitscan(rage);
        
        processed++;
        
        // Early termination if we found a lethal target
        if (!rage->points_to_scan.empty()) {
            bool found_lethal = false;
            for (const auto& point : rage->points_to_scan) {
                if (point.damage >= player->health()) {
                    found_lethal = true;
                    break;
                }
            }
            if (found_lethal) break; // Prioritize this lethal target
        }
    }
}
...
void c_ragebot::choose_best_point()
{
	...
	LISTENER_ENTITY->for_each_player([&](c_cs_player* player)
	{
		...
		auto get_best_aim_point = [&]() -> rage_point_t
		{
			...
			{
				auto is_body = point.hitbox == HITBOX_PELVIS || point.hitbox == HITBOX_STOMACH || point.hitbox == HITBOX_CHEST;
				...
				// PRIORITIZE SAFE POINTS FIRST
				if (point.safety == 5) // Perfect safe point
				{
					point.found = true;
					return point;
				}
				else if (point.safety >= 4 && point.damage >= player->health()) // Good safety + lethal
				{
					point.found = true;
					return point;
				}
				else if (is_body && point.damage >= player->health() && point.safety >= 3) // Body lethal with decent safety
				{
					point.found = true;
					return point;
				}
				else if (point.safety >= 3) // Decent safety
				{
					if (!best_safe.found || point.damage > best_safe.damage)
					{
						best_safe = point;
						best_safe.found = true;
					}
				}
				else if (point.safety >= 2) // Only risky shots with high damage
				{
			...
			// Return best safe point if available, otherwise best risky point
			return best_safe.found ? best_safe : best;
		};
...
{
	vec3_t origin = record ? record->origin : player->get_abs_origin();
	vec3_t abs_angles = record ? record->abs_angles : player->get_abs_angles();

	vec3_t delta = origin - HACKS->local->get_eye_position();
	delta.z = 0.f;
	delta = delta.normalized();

	vec3_t forward;
	math::angle_vectors(abs_angles, forward);
	forward.z = 0.f;
	forward = forward.normalized();

	float dot = delta.dot(forward);
	return dot > 0.475f; // higher threshold **may** increase the accuracy
}
...
bool c_ragebot::knife_trace(vec3_t dir, bool stab, c_game_trace* trace)
{
	...
	// trace additional hitboxes in case the first trace fails
	if (trace->fraction >= 1.f)
	{
		HACKS->engine_trace->trace_ray(ray_t(start, end, vec3_t(-16.f, -16.f, -18.f), vec3_t(16.f, 16.f, 18.f)), MASK_SOLID, &filter, trace);
	}
...
bool c_ragebot::can_knife(c_cs_player* player, anim_record_t* record, vec3_t angle, bool& stab)
{
	...
	c_game_trace trace;
	if (!knife_trace(forward, false, &trace))  // check for a slash
		return false;
	...
	bool first_swing = HACKS->weapon->next_primary_attack() + 0.4f < HACKS->predicted_time;
	bool backstab = knife_is_behind(player, record);

	int stab_dmg = knife_dmg.stab[armor][backstab];
	int slash_dmg = knife_dmg.swing[first_swing][armor][backstab];
	int swing_dmg = knife_dmg.swing[false][armor][backstab];
	...
	if (stab && !knife_trace(forward, true, &trace))  // check for a stab
		return false;
...
void c_ragebot::run()
{
	...
	if (best_rage_player.player && best_point.found && best_rage_player.start_scans)
	{
		...
		// QUALITY CHECK - don't take bad shots
		if (best_point.safety < 2 && best_rage_player.best_record->break_lc) {
			return; // Skip low safety + lagcomp break shots
		}
		
		if (missed_shots[best_rage_player.player->index()] >= 3 && best_point.hitbox == HITBOX_HEAD) {
			return; // Stop headshot attempts after 3 misses
		}
		...
		auto damage = this->get_min_damage(best_rage_player.player);
		...
		if (HACKS->cmd->buttons.has(IN_ATTACK))
		{
			...
			{
				EVENT_LOGS->push_message(tfm::format(CXOR("Fired shot at %s [hitbox: %s | hc: %d | sp: %d | dmg: %d | tick: %d | resolver: %s | side: %d | break lc: %s]"),
					...
					resolver_info[best_rage_player.player->index()].mode,
					resolver_info[best_rage_player.player->index()].side,
					best_record->break_lc ? "true" : "false"), {}, true);
			}
			...
			if (studio_model)
			{
				...
				else
				{
					if (new_player->is_alive())
					{
						...
						else if (shot.record.break_lc)
							EVENT_LOGS->push_message(XOR("Missed shot due to backtrack failure"));
...
void c_ragebot::collect_damage_from_multipoints_sync(int damage, vec3_t& predicted_eye_pos, 
    rage_player_t* rage, rage_point_t& points, anim_record_t* record, matrix3x4_t* matrix_to_aim, bool predicted)
{
    auto multipoints = this->get_points(rage->player, points.hitbox, matrix_to_aim);
    if (multipoints.empty())
        return;
    ...
    for (auto& multipoint : multipoints) {
        if (multipoint.second) continue; // Skip center points
        if (processed_points >= 4) break; // Limit multipoints for performance
        
        // Quick duplicate check
        bool already_exists = false;
        for (const auto& existing : rage->points_to_scan) {
            if (multipoint.first.dist_to(existing.aim_point) < 2.0f) {
                already_exists = true;
                break;
            }
        }
        ...
        auto bullet = penetration::simulate(HACKS->local, rage->player, start_eye_pos, multipoint.first);
        if (bullet.damage < damage || bullet.traced_target != rage->player || 
            (HACKS->weapon->is_taser() && bullet.penetration_count < 4))
            continue;
        ...
        // Quick safety calculation (simplified)
        point.safety = 3; // Default safety, you can improve this later
        
        rage->points_to_scan.emplace_back(point);
        processed_points++;
        
        // Early exit for lethal damage
        if (bullet.damage >= rage->player->health()) {
            break;
        }
    }
...
bool c_ragebot::is_lethal_damage(int damage, c_cs_player* player) 
{
    return damage >= player->health();
}

bool c_ragebot::should_force_body_aim(c_cs_player* player)
{
    if (g_cfg.binds[force_body_b].toggled) return true;
    
    auto estimated_body_damage = estimate_body_damage(player);
    return estimated_body_damage >= player->health();
}